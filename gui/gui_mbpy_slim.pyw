...
from mbpy.mb_poll import modbus_poller  # folder.file import function
...
class InputApp:
    ...
    def killframe(self):
        ...
        self.ch_gt.configure(state=DISABLED)
    ...
    def all_chk(self):
        if self.ip_gd and self.dev_gd and self.sreg_gd and self.lreg_gd and self.pd_gd:
            self.b_start.configure(state=NORMAL)
        else:
            self.b_start.configure(state=DISABLED)
...
class DisplayApp:
    ...
    #     self.l_val_polls_g = Label(self.gb_frm, text='Valid Polls: 1', anchor=W, width=15)
    #
    ...
    # variables
        ...
        self.tm = 101  # amount of time between queue checks
        self._job = None
        self.totpolls = 0
        self.valpolls = 0
    ...
    def mk_lbls(self, strt, cnt):
        ...
        # else:
        frm = self.tl_frm  # unindented for slim version
        rw_adj = 0  # unindented for slim version
        col_adj = 10  # unindented for slim version
    ...
    def write_lbls(self, data):
        if len(self.text_lbls) == len(data):
            ...
            self.otpt[-1].append(datetime.now())  # slim version of previous line
