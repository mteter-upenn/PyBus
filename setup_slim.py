...
gui_mbpy_slim = Target(
    ...
    script="gui_mbpy_slim.pyw", # path of the main script

    # Allows to specify the basename of the executable, if different from 'gui_mbpy_slim'
    dest_base = "PyBus Modbus Scanner Slim",

    # Icon resources:[(resource_id, path to .ico file), ...]
    # icon_resources=[(1, r"gui_mbpy_slim.ico")]
    # icon_resources = [(1, r"resources/favicon.ico")],
	icon_resources = [(1, r"resources/Upenn16.ico")],

    other_resources = [(RT_MANIFEST, 1, (manifest_template % dict(prog="gui_mbpy_slim", level="asInvoker")).encode("utf-8")),
...
##    dll_excludes = "MSVCP90.dll mswsock.dll powrprof.dll".split(),
    ...
    bundle_files=2,
    dist_dir='PyBus Modbus Scanner Slim',
    )
...
setup(name="name",
      ...
      # console=[gui_mbpy_slim],

      # windows subsystem executables (no console)
      windows=[gui_mbpy_slim],
